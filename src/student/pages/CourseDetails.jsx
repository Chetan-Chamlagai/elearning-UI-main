...
import { toast } from "react-toastify"; // NOTE: assumes <ToastContainer /> is already mounted at app root
import {
  ArrowLeft,
  ArrowRight,
  Star,
...
import { isFreeEnrolled, addFreeEnrollment } from "../utils/freeEnrollment";
...
/** A course is treated as free if its type says so OR price is 0/empty. */
const resolveIsFree = (category) => {
  const type = category?.categoryType?.toLowerCase?.();
  const price = Number(category?.price);
  return type === "free" || !price || price <= 0;
};

const getEnrollLabel = ({ isEnrolled, isFree, isProcessing, price }) => {
  if (isProcessing) return "Enrolling…";
  if (isEnrolled) return "Enrolled — Go to My Courses";
  return isFree ? "Enroll Now — Free" : `Enroll Now — ${formatPrice(price)}`;
};

/* ============================================================
 * Hero
 * ============================================================ */
const CourseHero = ({ category, stats, imageFetcher, enrollState, onEnroll }) => {
  ...
  const { isEnrolled, isFree, isProcessing } = enrollState;
    ...
    >
      ...
      <div className="course-hero__content">
        ...
        <div className="course-hero__actions">
          <button
            className={`course-hero__enroll ${isEnrolled ? "course-hero__enroll--done" : ""}`}
            onClick={onEnroll}
            disabled={isProcessing}
          >
            {isEnrolled && <CheckCircle2 size={16} />}
            {getEnrollLabel({ isEnrolled, isFree, isProcessing, price })}
          </button>
        </div>
 ...
 * Only shown for PAID, un-enrolled courses — free courses skip
 * straight to enrolled, so this banner never applies to them.
 * ============================================================ */
...
const CourseSidebar = ({ category, stats, onBack, enrollState, onEnroll }) => {
  ...
  return (
    ...
    >
      <div className="course-sidebar__card">
        {isEnrolled && (
          <div className="course-sidebar__enrolled-tag">
            <CheckCircle2 size={15} /> You're enrolled in this course
          </div>
        )}
        ...
        <button
          className={`course-sidebar__enroll ${isEnrolled ? "course-sidebar__enroll--done" : ""}`}
          onClick={onEnroll}
          disabled={isProcessing}
        >
          {getEnrollLabel({ isEnrolled, isFree, isProcessing, price })}
 ...
 *
 * Enrollment:
 *  - FREE courses: clicking "Enroll Now" never touches the payment API.
 *    It's recorded instantly via utils/freeEnrollment (localStorage) and
 *    the student is immediately treated as enrolled.
 *  - PAID courses: unchanged — "Enroll Now" routes to /student/payment/:id,
 *    and enrollment status comes from useEnrollmentStatus (APPROVED payment).
 *
...
const CourseDetails = () => {
  ...
  // Local, instant "am I free-enrolled" flag — separate from the paid
  // enrollment hook below since it never touches the backend.
  const [isFreeEnrolledLocal, setIsFreeEnrolledLocal] = useState(false);
  const [isEnrolling, setIsEnrolling] = useState(false);

  // Paid-course enrollment (APPROVED payment) still comes from the API.
  const { isEnrolled: isPaidEnrolled } = useEnrollmentStatus(categoryId);

  const isFree = resolveIsFree(category);
  const isEnrolled = isFree ? isFreeEnrolledLocal : isPaidEnrolled;

  useEffect(() => {
    setIsFreeEnrolledLocal(isFreeEnrolled(userId, categoryId));
  }, [userId, categoryId]);
  ...
  const handleEnroll = () => {
    // Already enrolled → button becomes a shortcut to My Courses.
    if (isEnrolled) {
      navigate("/student/my-courses");
      return;
    }

    // Paid course → unchanged, goes through the payment page/API.
    if (!isFree) {
      navigate(`/student/payment/${categoryId}`);
      return;
    }

    // Free course → no payment API. Save locally and flip state instantly.
    setIsEnrolling(true);
    addFreeEnrollment(userId, category);
    setIsFreeEnrolledLocal(true);
    setIsEnrolling(false);
    toast.success(`Enrolled in "${category?.categoryTitle || "course"}"!`);
  };

  const enrollState = { isEnrolled, isFree, isProcessing: isEnrolling };
    ...
    <div className="course-details">
      ...
      <div className="course-details__body">
        <div className="course-details__main">
          ...
          {/* Free courses skip this banner entirely — enrolling is instant,
              so there's never a "locked, waiting on payment" state for them. */}
          {!isEnrolled && !isFree && activeTab !== "overview" && (
